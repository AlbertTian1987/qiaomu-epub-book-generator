...
0. **源材料预处理（必做）**
   - 如果输入是 `.zip`/`.rar` 文件 → **先解压到临时目录**
   - 扫描所有 `.md`/`.txt`/`.html` 文件，检测编码
   - **编码转换**：非 UTF-8 编码的文件（如 GBK/GB2312/BIG5）→ 用 `iconv` 或 Python 转换为 UTF-8
   - **所有后续流程假设文件为 UTF-8 编码**，不解码直接处理会出乱码
   
0. **信息确认（必做）**
   - 确定书名：从源文件目录名或用户告知获取
   - **确定作者**：如果源数据中不包含作者信息，**必须询问用户**「作者是谁？」，不得跳过或留空
   - **确定封面**：询问用户「是否已有封面图片？还是自动生成？」
     - 有封面图片 → 确认图片路径，用 `--cover` 参数
     - 无封面图片 → 用 `--cover-svg` 自动生成（推荐）
   - 确认后生成命令，让用户确认后再执行
...
⓪-a 源材料预处理
    - zip/rar → 解压到临时目录
    - 检测编码，非 UTF-8（GBK/BIG5 等）→ 转为 UTF-8
⓪-b 信息确认
   - 书名：从目录名或用户告知获取
   - 作者：如果源数据中没有 → 问用户「作者是谁？」
   - 封面：问用户「已有封面图片还是自动生成？」
    ↓
① 分析原始材料结构（打开样本看规律）
    ↓
