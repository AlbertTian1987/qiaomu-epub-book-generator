...
|---|---|
| 羊皮纸底，永不纯白 | 强制 `body { background: var(--paper) }` → **2026-05-03 废弃**：跨阅读器兼容性差，改由阅读器控制背景，文字色仍受约束 |
...
**原本强制 `body { background: var(--paper) }`，现改为不设任何背景。**

理由：Apple Books / Kindle 对自定义背景色的支持差异极大——部分阅读器会覆盖、部分会裁剪、部分会忽略。实际测试中 Apple Books 的背景渲染不可靠（中后段章节背景消失），且读者通常有自己偏好的阅读主题（浅色/深色/羊皮纸）。EPUB 不应剥夺读者的选择权。

**我们仍然为自己的文字颜色负责**——Kami 色板中的墨蓝、暖灰、stone、olive 在所有光照条件下都经过设计，但背景色交由阅读器控制。

承认妥协：部分阅读器（Calibre）会尊重我们的背景设置，但苹果生态不支持。跨阅读器一致性比强加背景更重要。
...
- ~~§2 设计契约第一行原 EPUB 落地「正文不设 body 背景」~~ → 曾改为强制背景 + 双光照色板 → **2026-05-03 废弃**：背景交由阅读器控制
...
- **不让阅读器决定背景**——~~已废弃，见 §2.5~~ 阅读器控制背景，我们控制文字色
...
- ~~原 §2 设计契约「正文不设 body 背景」~~ → 曾改为强制背景 + var(--paper) → **2026-05-03 废弃**：阅读器兼容性差，改回不设背景
...
| 2026-05-03 | 废弃强制背景，改由阅读器控制 | Apple Books 背景渲染不可靠，跨阅读器一致性更重要 |
